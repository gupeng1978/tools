...
class Table_Attr:
    ...
    def __set_header_attr(self, row_start, row_end):
        # 设置居中对齐属性
        alignment = Alignment(horizontal='center', vertical='center')        

        # 遍历指定行范围的所有单元格
        for row in self.__worksheet.iter_rows(min_row=row_start, max_row=row_end):
            for cell in row:
                # 设置居中对齐属性
    ...
    def __set_record_attr(self, row_start, row_end):
    ...
    def __set_col_cell_width(self, row_start, row_end):
        # 初始化一个列表来存储每一列的最大字符宽度
        max_col_widths = [0] * self.__worksheet.max_column

        # 遍历指定行范围的所有单元格
        for row in self.__worksheet.iter_rows(min_row=row_start, max_row=row_end, values_only=True):
        ...
        for col_index, col_width in enumerate(max_col_widths):
            ...
            # 设置列宽
            self.__worksheet.column_dimensions[chr(col_index + 65)].width = adjusted_width
    ...
    def set_attr(self):        
        if 'header' in self.__table_info:
            ...
            self.__set_header_attr(header_info['row_start'], header_info['row_end'])
                
       
        if 'record' in self.__table_info:
            record_info = self.__table_info['record']
            self.__set_record_attr(record_info['row_start'], record_info['row_end'])
            
        if 'header' in self.__table_info and 'record' in self.__table_info:
            self.__set_col_cell_width(header_info['row_start'], record_info['row_end'])
